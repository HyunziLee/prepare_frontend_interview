...
=======
  > 프로세스는 해당 프로그램을 실행하여 메모리 상에서 실행되는 작업단위를 뜻함.
>>>>>>> 6791ee8273058cedb4699e0f9ec925f05ab0ed22
  - 스레드가 뭔가요?
...
- [싱글 스레드와 멀티 스레드 🔥](#싱글-스레드와-멀티-스레드)
  ...
  - 멀티스레드
  > CPU에서 여러 프로세스를 돌아가며 작업을 처리, 위에서 말한 보류와 깨움으로 이루어진 일련의 작업입니다.
...
- [HTTP 🔥](#HTTP)
  ...
  > HTTP/1.1은 Connection 한 개당 하나의 요청을 처리, HTTP/2.0은 connection 한 개로 동시에 여러 개의 메시지를 주고 받을 수 있으며, 응답은 순서에 상관없이 stream으로 주고 받는다. 
...
- [웹팩 🔥](#웹팩)
  ...
  - 바벨이란?
  > 바벨(babel)은 트랜스파일러(transpiler)로, '구 표준'을 준수하는 코드로 변환해준다
