...
- [변수 🔥](#변수)
  ...
  - 변수란 무엇인가요?
    ...
    - 값을 저장하기 위해 확보한 메모리 공간, 메모리 공간을 식별하기 위해 붙인 이름

  - 식별자란 무엇인가요? 🔥

    - 변수 이름을 식별자라고 함, 식별자는 값이 아니라 메모리 주소를 기억함

  - 변수를 선언한다는 것은 어떤 것을 의미하나요?

    - 값을 저장하기 위한 메모리 값을 확보
    - 변수 이름과 확보한 메모리 공간의 주소를 연결

  - var 키워드는 뭔가요?

  - 호이스팅이 뭔가요? 🔥🔥

    - 변수 선언을 변수 범위 맨위로 끌고 오는 현상
  ...
  - 식별자 네이밍 규칙은 어떤 것들이 있나요?
    ...
    - 특수문자를 제외한 문자, 숫자, \_, $를 포함할 수 있음.
    - 특수문자를 제외한 문자, \_, $로 시작해야함
    - 예약어는 식별자로 사용할 수 없음 (new, const, var, let, catch 등)
  ...
  - 리터럴이 뭔가요?
    - 객체, 배열을 정의하는 간단한 방법 (new 키워드 안써도 됨)
...
- [구조 분해 할당 🔥](#구조-분해-할당)
  ...
  - 구조 분해 할당이 뭔가요?
    - 객체, 배열 값을 해체한 후 개별 값을 변수에 새로 할당하는 과정을 말한다.
    ```
    let a = [1, 2, 3, 4];
...
- [제너레이터와 async await 🔥](#제너레이터와-async-await)
  ...
  - Promise와 async/await의 차이점 한 줄 요약 🔥
    - 에러를 핸들링 할 수 있는 기능 여부, async/await는 try-catch() 문을 활용하여 에러를 핸들링 하여 주어야 한다.

- [에러 🔥](#에러)

  - 에러처리를 왜 해야 하나요? -코드가 멈추기 때문(유저 화면에 이상이 생김)
