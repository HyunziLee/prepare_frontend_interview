...
- [변수 🔥](#변수)
  ...
  - 변수를 선언한다는 것은 어떤 것을 의미하나요?
   - 메모리 공간을 확보하고, 변수 이름과 확보된 메모리 주소를 연결하는 것

  - var 키워드는 뭔가요?
    - 중복선언, 재할당이 가능한 키워드 

  - 호이스팅이 뭔가요? 🔥🔥
    - 변수, 함수의 선언을 변수 범위 맨위로 끌고가서 가장 먼저 해석하는 것

  - var 키워드의 문제점은 무엇이 있나요? 🔥
    - 중복선언, 재할당이 가능하다.

  - let 키워드는 var 키워드와 어떤 점이 다른가요? 🔥🔥
    - 중복 선언 여부 (var: 가능, let: 불가능)
    - TDZ
  ...
  - 식별자 네이밍 규칙은 어떤 것들이 있나요?
    ...
    - 특수문자를 제외한 문자, 언더스코어, 달러 기호로 시작해야함
    - 예약어는 식별자로 사용할 수 없음 (ex await, break, if 등)

  - 네이밍 컨벤션은 어떤 것들이 있나요?
    - 카멜 케이스 ex) userInfo
    - 스네이크 케이스 ex) user_info
    - 파스칼 케이스 ex) UserInfo

  - 리터럴이 뭔가요?
    - 자료를 표기하는 방식 ( 변수 선언과 동시에 값을 할당하는 것)
...
- [생성자 함수에 의한 객체 생성 🔥](#생성자-함수에-의한-객체-생성)
  ...
  - 생성자 함수가 뭔가요?
    - 객체를 생성하는 함수 ex) const date = new Date()

  - 객체 리터럴로 만들 때와는 무슨 차이가 있죠? 왜 생성자 함수를 사용하나요? 
    - 자식들이 값을 직접 소유하게 만들고 싶으면 constructor(생성자)로 상속시키시면 되고, 부모만 가지고 있고 그걸 참조해서 쓰게 만들고 싶으면 prototype으로 상속시키면 됨
...
- [스프레드 문법 🔥](#스프레드-문법)
  ...
  - 어떤 상황에서 사용할 수 있죠?
    - 배열, 객체는 복사를 하면, 메모리 주소를 공유하게 되는데 (배열, 객체는 변수에 메모리 주소값을 저장하기 때문에 ) 이 때, 복사된 값을 변경하게 되면 원본 배열 또는 객체도 변경된다. 이 때 spread 연산자를 사용하면, 메모리 값이 아닌 값만 복사할 수 있다. 

- [구조 분해 할당 🔥](#구조-분해-할당)

  - 구조 분해 할당이 뭔가요?
    - 배열, 객체에서 각각 값(value)이나 프로퍼티(property)를 분해하여 손쉽게 별도의 변수에 담을 수 있도록 함
...
- [Ajax 🔥](#Ajax)
  ...
  - Ajax가 뭔가요 어떤 것을 담당하고 있죠?
    - 새로고침 없이 get/post/put/delete 요청
