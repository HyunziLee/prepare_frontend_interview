...
- [데이터 타입 🔥](#데이터-타입)
  ...
  - 원시타입: 원시타입의 값은 변경 불가능한 값 (immutable value)이며, 값에 의한 전달
    - string
    - boolean
    - null
    - undefined
    - symbol
    - number
  - 객체타입: 객체 참조에 의한 절달 방식으로 전달

    - object

  - 심벌 타입은 뭐죠?

    - 변경 불가능한 원시타입 값, 주로 이름 충돌 위험이 없은 유일한 객체의 property key를 위해 사용

  - 데이터 타입은 왜 필요할까요? 🔥

    - 값을 저장할 때 필요한 메모리 공간의 크기를 알기 위해
    - 값을 참조할 때 한번에 읽어 들일 메모리 공간의 크기를 알기 위해
    - 메모리에서 읽어 들인 2진수를 어떻게 해석할지 알기 위해

  - 정적 타이핑이 뭔가요?

    - c언어 처럼 변수를 선언할 때, 타입을 지정하는 것

  - 동적 타이핑이 뭔가요?
    - 변수를 선언할 때 타입을 직접적으로 지정하지 않는 것
    - 런타임에 타입 지정이 실행 됨(변수를 선언할 때가 아닌, 할당할 때 타입이 정해짐)

- [타입변환과 단축 평가 🔥](#타입변환과-단축-평가)

  - 명시적 타입 변환이 뭔가요?

    - 내장 객체를 이용해 명시적으로 타입을 변경하는 것
  ...
  - 암묵적 타입 변환이 뭔가요?
    ...
    - 개발자의 의도환 상관없이 js 엔진에 의해 암묵적으로 타입지 자동으로 변환되는 것
      - console.log("10" + 1); // '101'
      - console.log(10 \* "10"); // 100

  - truthy / falsy 한 값이 뭔가요?
    - true 같은 값, false 같은 값
    - truthy
      - falsy한 값이 아닌 전부
    - falsy
      - 0
...
- [객체 리터럴 🔥](#객체-리터럴)
  ...
  - 함수와 메서드의 차이점에 대해 알고 계신가요?
    ...
    - '객체로부터 독립적인가 아닌가'가 함수와 메소드를 나누는 기준이다.
...
- [원시 값과 객체 비교 🔥](#원시-값과-객체-비교)
  ...
  - 값에 의한 전달이 뭔가요? 🔥
    ...
    - 변수에 원시 값을 갖는 변수를 할당하면 할당 받는 변수에는 할당하는 변수의 원시 값이 복사되어 전달된다.

  - 참조에 의한 전달이 뭔가요? 🔥
    - 객체는 원시 값과 같이 확보해야 할 메모리 공간의 크기를 사전에 정해 둘 수 없음 ➡️ 객체의 가변성 성질 때문에 객체를 할당한 변수가 기억하는 메모리 주소를 통해 메모리 공간에 접근해 참조 값에 접근해야 함 (참조 값 = 객체가 저장된 메모리 공간의 주소)
...
- [생성자 함수에 의한 객체 생성 🔥](#생성자-함수에-의한-객체-생성)
  ...
  - 생성자 함수가 뭔가요?
    - 붕어빵틀 역활
    - new 키워드를 이용해서 인스턴스 만들 때 사용
  - 객체 리터럴로 만들 때와는 무슨 차이가 있죠? 왜 생성자 함수를 사용하나요?
    - 자식들이 값을 직접 소유하게 만들고 싶으면 constructor로 상속시키시면 되고 부모만 가지고 있고 그걸 참조해서 쓰게 만들고 싶으면 prototype으로 상속시키면 됨
...
- [함수와 일급 객체 🔥](#함수와-일급-객체)
  ...
  - 꼬리 질문) 함수형 프로그래밍이 뭔가요? 🔥
    - 대입문이 없는 프로그래밍 = 메모리에 할당된 값은 새로운 값으로 변할 수 없음 = 항상 동일한 결과를 반환하는 순수 함수로 이주어진 프로그래밍
  - 꼬리 질문) 순수 함수가 뭔가요? 일반 함수와는 어떤 차이가 있죠? 🔥
    - 외부의 개입 없이 동일한 인풋값에 따라 항상 동일한 아웃풋을 반환하는 함수
      순수함수
      ```
...
- [this 🔥](#this)
  ...
  - this가 뭔가요? 🔥
    ...
    - this가 무엇인지는 함수를 호출한 코드에 달려있으며, 함수가 호출될 때 동적으로 결정되기 때문에

  - this 바인딩이란? 🔥

    - this(식별자 역할)와 this가 가리킬 객체를 바인딩하는 것이다.
...
- [클로저 🔥](#클로저)
  ...
  - 클로저에 대해서 아나요? 🔥🔥
    ...
    - 어떤 함수가 자신의 내부가 아닌 외부에서 선언된 변수에 접근(참조)하는 것

  - 클로저를 사용하면 뭐가 좋죠? 🔥

    - 외부에서는 함수에 있는 변수 접근이 불가능 하기때문에 캡슐화를 구현하여 데이터를 은닉할 수 있음
    - 함수가 호출이 끝나도 내부 함수가 있기 때문에 계속 해서 메모리에서 지워지지 않고 유지가 되어 상태를 유지 할 수 있음
    - 함수를 객체처럼 새로 만들어 사용할 수도 있음
...
- [REST API 🔥](#REST-API)
  ...
  - HTTP 요청 메서드에 대해서 아는대로 얘기해보세요
    ...
    - put: 수정
    - delete: 삭제
    - head: 서버 정보 확인 GET과 동일하지만, response에 Body가 없고 response Code와 Head만 응답받는다.
...
- [제너레이터와 async await 🔥](#제너레이터와-async-await)
  ...
  - async/await 가 뭔가요? 기존의 Promise와는 어떤 차이가 있죠? 🔥
    ...
    - async, await
    - async 쓰면 Promise 오브젝트가 그냥 생성됨 (단, function 키워드 앞에서만 가능)
      - 이 함수 실행하면 그 자리에 Promise 인스턴스로 만든 오브젝트가 남음 그래서, 이 함수 뒤에 then() 쓸 수 있음
    - await: 어려운 연산 Promise를 기다린 다음에 완료되면,결과를 ~~ 해주세요 (단, async 쓴 함수 안에서만 사용 가능
      - then() 대체품
      - await은 실패하면 에러나고 코드 멈춤 -> try{} catch{} 사용해야함

  - Promise와 async/await의 차이점 한 줄 요약 🔥
    - 에러 핸들링 여부 async/await 은 에러 나면 코드가 멈추기 때문에 try/catch 사용해서 에러 처리함
